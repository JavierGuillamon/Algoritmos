...
double Matriz::calcularDeterminanteRecursivo(const Matriz &m) {
	if (m.n_columnas == 2) {
		...
		{
			Matriz *temp = new Matriz(m.n_columnas -1, m.n_columnas -1);
			int a = 0, b = 0;
			for (int k = 0; k < m.n_columnas; k++) {
				if (k == 0) {}
				else {
					for (int h = 0; h < m.n_columnas; h++) {
						if (h == i) {}
						else {
							temp->matriz[a][b] = m.matriz[k][h];						
							if (b == m.n_columnas - 2) b = 0;
							else b++;
						}
					}
					a++;
			...
			if (i % 2 == 0) {		
				aux += m.matriz[0][i]*calcularDeterminanteRecursivo(*temp);;
			}
			else {
				aux -= m.matriz[0][i] * calcularDeterminanteRecursivo(*temp);
			}
...
double Matriz::calcularDeterminante() {
	return calcularDeterminanteRecursivo(*this);
}
