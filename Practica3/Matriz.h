...
public:
	...
	*/
	...
	Precondiciones: n_filas=n_columnas
	Complejidad:
	-Temporal: O(n)=n^2 -> Constante oculta T(n)= (n^2)/2
	-Espacial: O(n)=1
	*/
	bool esSimetrica();
	...
	*/
	...
	Retorno: valor del determinante de la matriz
	*/
	double calcularDeterminante();

	/*
	Metodo recursivo para calcular el determinante de una matriz dada mediante el método de los adjuntos
	Parámetros:
	-m: matriz de la que se quier calcular el determinante
	Retorno: el valor del determinante
	*/
	double calcularDeterminanteRecursivo(const Matriz &m);
private:
